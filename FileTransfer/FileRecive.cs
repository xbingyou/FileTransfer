...
namespace FileTransfer
{
    class FileRecive
    {
        ...
        public FileRecive(frmFile fm,string strIp,int nPort)
        {
            this.fm = fm;
            try
            {
                this.server = new TcpListener(IPAddress.Parse(strIp), nPort);
                server.Start();
            }
            catch (Exception e)
            {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (DialogResult.Yes == MessageBox.Show(IpUtil.GetLocalIp() + "给您发了一个文件:" + tip[0] + "大小为:" + (long.Parse(tip[1]) / 1024) + "kb ,确定要接收吗?", "接收提醒", MessageBoxButtons.YesNo))
                    {
                        ...
                        fm.SetState("正在接收：");
                        //开始接收文件
                        string path = System.AppDomain.CurrentDomain.BaseDirectory + tip[0];//接收文件的存储路径
